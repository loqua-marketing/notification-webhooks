...
import com.nunomonteiro.notificationwebhooks.service.MLService
import com.nunomonteiro.notificationwebhooks.worker.WebhookWorker
    ...
    fun executarWebhook(regra: Rule, notification: Notification) {
        coroutineScope.launch {
            try {
                ...
                // Observar o resultado do worker
                workManager.enqueue(request)
                
                // Opcional: Observar quando o worker termina para atualizar UI
                workManager.getWorkInfoByIdLiveData(request.id)
                    .observeForever { workInfo ->
                        if (workInfo != null && workInfo.state.isFinished) {
                            // Quando o worker terminar, recarregar a lista
                            coroutineScope.launch {
                                carregarPrimeiraPagina(currentFilterType)
                            }
                        }
