...
class NotificationListener : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        ...
        Log.i(TAG, "📨 Notificação recebida: $packageName - $title")
        
        // IMPORTANTE: Chamar o repositório para guardar
        CoroutineScope(Dispatchers.IO).launch {
            try {
                repository.insertNotification(packageName, title, text)
                Log.d(TAG, "✅ Notificação inserida no repositório")
            } catch (e: Exception) {
                Log.e(TAG, "❌ Erro ao inserir notificação", e)
            }
        }
    }
